...
class Recent_Posts_Widget_With_Thumbnails extends WP_Widget {
	...
	function __construct() {
        ...
        parent::__construct(
            ...
            esc_html__('Recent Posts With Thumbnails', 'ailab'), // Name
            array('description' => esc_html__('List of your site&#8217;s most recent posts, with clickable title and thumbnails.', 'ailab'),) // Args
        );
				...
				<div class="tb-recent-detail">
                      ...
                      <?php if ( ! $hide_thumbnail ) { ?>
                        ...
                        if(has_post_thumbnail()) {
                                ...
                                if (function_exists('jws_getImageBySize')) {
                                    ...
                                    $img = jws_getImageBySize(array('attach_id' => $attach_id, 'thumb_size' => '150x150', 'class' => 'image-blog'));
                                    echo ''.$img['thumbnail'];
                                    }else {
                                    echo ''.$img = get_the_post_thumbnail(get_the_ID(), '150x150');
                                  }
                          }                                 
                    ...
                    <?php if ( ! $hide_author ) { ?>
						<div class="tb-post-author">
							<p>		
								<?php 
								echo esc_html__('By ', 'ailab').'<span>'.get_the_author().'</span>'
								?>
							</p>
					...
					<?php if ( ! $hide_button ) { ?>
						<!-- Btn read mor -->
						<a href="<?php the_permalink(); ?>" class="btn btn-default"><?php esc_html_e('Read More', 'ailab');?></a>
					</div>	
					<?php }?>
				</div>				
		<?php endwhile; ?>
...
<?php
	...
	function form( $instance ) {
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'hide_thumbnail' )); ?>"><?php esc_html_e( 'Do not show hide_thumbnail?', 'ailab' ); ?> <em><?php esc_html_e( 'Make sure you set a default thumbnail for posts without a tille.', 'ailab' ); ?></em></label> </p>
        
		<p><label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'ailab' ); ?></label>
		<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></p>
        
		<p><label for="<?php echo esc_attr($this->get_field_id( 'number_posts' )); ?>"><?php esc_html_e( 'Number of posts to show:', 'ailab' ); ?></label>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'show_date' )); ?>"><?php esc_html_e( 'Display post date?', 'ailab' ); ?></label></p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'hide_title' )); ?>"><?php esc_html_e( 'Do not show title?', 'ailab' ); ?> <em><?php esc_html_e( 'Make sure you set a default title for posts without a tille.', 'ailab' ); ?></em></label> </p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'hide_author' )); ?>"><?php esc_html_e( 'Do not show author?', 'ailab' ); ?> <em><?php esc_html_e( 'Make sure you set a default author for posts without a tille.', 'ailab' ); ?></em></label> </p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'hide_content' )); ?>"><?php esc_html_e( 'Do not show content?', 'ailab' ); ?> <em><?php esc_html_e( 'Make sure you set a default content for posts without a content.', 'ailab' ); ?></em></label> </p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'hide_button' )); ?>"><?php esc_html_e( 'Do not show button read more?', 'ailab' ); ?> <em><?php esc_html_e( 'Make sure you set a default button for posts without a button.', 'ailab' ); ?></em></label> </p>
...
function zo_register_recent_posts_widget_with_thumbnails () {
    if(function_exists('insert_widgets')) {
        insert_widgets('Recent_Posts_Widget_With_Thumbnails');
    }
}
