 ...
 * @since 1.2.0
 */
class Jws_Breakscrumbs extends Widget_Base{
	...
	public function get_title() {
		return __( 'Jws Breadcrumbs', 'ailab' );
	}
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				[
					...
					'default'	=> '',
					'options'	=> [
						''		=> __( 'Current page', 'ailab' ),
						'id'	=> __( 'Specific page', 'ailab' ),
					]
				]
			);
			...
			$this->add_control(
				'cpt_crumbs',
				[
					'label' 		=> __( 'CPT Crumbs', 'ailab' ),
					'type' 			=> Controls_Manager::SELECT,
					'default' 		=> '',
					'options'		=> [
						'' 			=> __( 'CPT Name', 'ailab' ),
						'terms' 	=> __( 'Taxonomy Terms', 'ailab' ),
						'both' 		=> __( 'Both', 'ailab' ),
					],
				]
			);
			...
			$this->add_control(
				'home_text',
				[
					'label' 		=> __( 'Home Text', 'ailab' ),
					'type' 			=> Controls_Manager::TEXT,
					'default' 		=> __( 'Homepage', 'ailab' ),
					'dynamic'		=> [
						'active'	=> true,
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				[
					...
					'selectors' => [
						'{{WRAPPER}} .jws-breadcrumbs' => 'margin-left: -{{SIZE}}{{UNIT}};',
			...
			$this->start_controls_tab( 'crumb_default', [ 'label' => __( 'Default', 'ailab' ) ] );
			...
			$this->start_controls_tab( 'crumb_hover', [ 'label' => __( 'Hover', 'ailab' ) ] );
		...
		$this->start_controls_section(
			...
			$this->add_control(
				...
				[
					...
					'default'	=> '',
					'selectors' => [
						'{{WRAPPER}} .jws-breadcrumbs__item--current' => 'color: {{VALUE}};',
					],
				]
			);
	...
	protected function render() {
		...
		if ( $_query ) {
			if ( $_query->have_posts() ) {
				...
				_e( 'Post or page not found', 'ailab' );
		...
		if ( ! $query->is_front_page() ) { ?>
			...
			if ( $query->is_archive() && ! $query->is_tax() && ! $query->is_category() && ! $query->is_tag() && ! $query->is_date() && ! $query->is_author() ) {
				...
				$this->render_item( 'tag', [
					...
					'content' 	=> sprintf( __( 'Tag: %s', 'ailab' ), $get_term_name ),
				] );
				...
				$this->render_item( 'year', [
					...
					'key' 		=> 'year',
					'ids' 		=> [ get_the_time('Y') ],
					'content' 	=> sprintf( __( '%s Archives', 'ailab' ), get_the_time('Y') ),
				...
				$this->render_item( 'month', [
					...
					'key' 		=> 'month',
					'ids' 		=> [ get_the_time('m') ],
					'content' 	=> sprintf( __( '%s Archives', 'ailab' ), get_the_time('F') ),
				...
				$this->render_item( 'day', [
					...
					'key' 		=> 'day',
					'ids' 		=> [ get_the_time('j') ],
					'content' 	=> sprintf( __( '%1$s %2$s Archives', 'ailab' ), get_the_time('F'), get_the_time('jS') ),
				] );
				...
				$this->render_item( 'author', [
					...
					'content' 	=> sprintf( __( 'Author: %s', 'ailab' ), $userdata->display_name ),
				] );
				...
				$this->render_item( 'search', [
					...
					'separator'	=> false,
					'key' 		=> 'search',
					'content' 	=> sprintf( __( 'Search results for: %s', 'ailab' ), get_search_query() ),
				] );
				...
				$this->render_item( '404', [
					...
					'separator'	=> false,
					'key' 		=> '404',
					'content' 	=> __( 'Page not found', 'ailab' ),
				] );
			}
