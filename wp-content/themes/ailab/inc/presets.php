<?php
/** proces less files and generate ailab-style.css in uploads folder */
{
    /**
    ...
    {
        global $ailab_less_files, $ailab_stylesheet_directory, $ailab_template_directory;

        $ailab_stylesheet_directory = get_stylesheet();
        $ailab_template_directory = get_template();
        ...
        /**
         * Less files are generated in one file ailab-style.css, in upload folder, after options are saved or if
         * ailab-style.css file doesn't exist yet
         */
        if (!file_exists($style_dir . '/jws-style.css') || $autosave ) {
            ...
            try {
                ...
                if (!function_exists('verify_less_in_child_theme')) {
                    ...
                    {
                        global $ailab_stylesheet_directory, $ailab_template_directory;

                        $evaldPath = $evald->PathAndUri();
                        $full_path = preg_replace('/' . $ailab_template_directory . '/', $ailab_stylesheet_directory, $evaldPath[0]);

                        if (file_exists($full_path)) {
                            $uri = preg_replace('/' . $ailab_template_directory . '/', $ailab_stylesheet_directory, $evaldPath[1]);
                ...
                if (class_exists('Less_Parser')) {
                    ...
                    $parser->parseFile($style_less, $ailab_template_directory_uri);
                    if (is_rtl()) {
                        $parser->parseFile($rtl_less, $ailab_template_directory_uri);
                    }
                    ...
                    include(JWS_ABS_PATH.'/inc/option-style.php');
                    $parser->ModifyVars($ailab_modified_variables);


                    /**
                     * Parse all css and write generated styles to ailab-style.css in upload folder,
                    ...
                    if (!$wp_filesystem->put_contents($style_dir . '/jws-style.css', $css)) {
                        esc_html_e("error write content", 'ailab');
                    }
                }
                ...
                /** show parser errors in admin */
                    ...
                    ?>
                    <div class="error">
                        <p><?php printf(esc_html__('lessphp fatal error:  %s', 'ailab'), $e->getMessage()); ?></p>
                    </div>
                    <?php
