...
class Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu
{
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $current_object_id = 0)
    {
        ...
        if (!empty($item->_invalid)) {
            ...
            $title = sprintf(esc_html__('%s (Invalid)', 'ailab'), $item->title);
        } elseif (isset($item->post_status) && 'draft' == $item->post_status) {
            $classes[] = 'pending';
            /* translators: %s: title of menu item in draft status */
            $title = sprintf(esc_html__('%s (Pending)', 'ailab'), $item->title);
        }
            ...
            <dt class="menu-item-handle">
                ...
                <span class="item-controls">
                    ...
                    <span class="item-order hide-if-js">
                        ...
                        ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'ailab'); ?>">&#8593;</abbr></a>
                        |
                        <a href="<?php
                        echo wp_nonce_url(
                            add_query_arg(
                        ...
                        ?>" class="item-move-down"><abbr
                                    title="<?php esc_attr_e('Move down', 'ailab'); ?>">&#8595;</abbr></a>
                    </span>
                    <a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>"
                       title="<?php esc_attr_e('Edit Menu Item', 'ailab'); ?>" href="<?php
                    echo (isset($_GET['edit-menu-item']) && $item_id == $_GET['edit-menu-item']) ? admin_url('nav-menus.php') : add_query_arg('edit-menu-item', $item_id, remove_query_arg($removed_args, admin_url('nav-menus.php#menu-item-settings-' . $item_id)));
                    ?>"><?php esc_html_e('Edit Menu Item', 'ailab'); ?></a>
                </span>
            </dt>
        </dl>
        ...
        <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('Navigation Label', 'ailab'); ?><br/>
            ...
            <p class="description description-thin">
                <label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('Title Attribute', 'ailab'); ?><br/>
            ...
            <p class="field-link-target description">
                <label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
                    ...
                    <?php esc_html_e('Open link in a new window/tab', 'ailab'); ?>
                </label>
            </p>
            <p class="field-css-classes description description-thin">
                <label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('CSS Classes (optional)', 'ailab'); ?><br/>
            ...
            <p class="field-xfn description description-thin">
                <label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('Link Relationship (XFN)', 'ailab'); ?><br/>
            ...
            <?php if (!$depth) { ?>
                <p class="field-description description description-wide">
                    <label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e('Add Shortcode for mega menu', 'ailab'); ?><br/>
                        ...
                        <span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.', 'ailab'); ?></span>
                    </label>
                </p>
            <?php }
            /*
                ...
                <p class="description description-thin">
                    ...
                    <label for="edit-<?php echo esc_attr($key . '-' . $item_id); ?>">
                        <select id="edit-<?php echo esc_attr($key . '-' . $item_id); ?>"
                            ...
                            <option value="standard" <?php echo ''.($value == 'standard') ? ' selected="selected" ' : ''; ?>><?php esc_html_e('Standard Dropdown', 'ailab'); ?></option>
                            <option value="mega_menu" <?php echo ''.($value == 'mega_menu') ? ' selected="selected" ' : ''; ?>><?php esc_html_e('Mega Dropdown', 'ailab'); ?></option>
                            <option value="mega_menu_full_width" <?php echo ''.($value == 'mega_menu_full_width') ? ' selected="selected" ' : ''; ?>><?php esc_html_e('Mega Full Width Dropdown', 'ailab'); ?></option>
                        </select>
                    </label>
                </p>
                <?php
            ...
            */
                ...
                <p class="description description-thin">
                    ...
                    <label for="edit-<?php echo esc_attr($key . '-' . $item_id); ?>">
                        <select id="edit-<?php echo esc_attr($key . '-' . $item_id); ?>"
                                class=" <?php echo esc_attr($key); ?>"
                                name="<?php echo esc_attr($key . "[" . $item_id . "]"); ?>">
                            <option value="center" <?php echo ''.($value == 'center') ? ' selected="selected" ' : ''; ?>><?php esc_html_e('Center', 'ailab'); ?></option>
                            <option value="left" <?php echo ''.($value == 'left') ? ' selected="selected" ' : ''; ?>><?php esc_html_e('Left', 'ailab'); ?></option>
                            <option value="right" <?php echo ''.($value == 'right') ? ' selected="selected" ' : ''; ?>><?php esc_html_e('Right', 'ailab'); ?></option>
                        </select>
                    </label>
                </p>
                <?php
            ...
            if (!$depth) {
                ...
                <p class="field-submenu_size description description-thin">
                    <label for="edit-menu-item-submenu_size-<?php echo esc_attr($item_id); ?>">
                        <?php esc_html_e('Add Width Mega Menu ', 'ailab'); ?><br/>
            ...
            <p class="field-menu_icon description description-thin">
                <label for="edit-menu-item-menu_icon-<?php echo esc_attr($item_id); ?>">
                    <?php esc_html_e('Add Icon Menu ', 'ailab'); ?><br/>
            ...
            <div class="menu-item-actions description-wide submitbox">
                <?php if ('custom' != $item->type && $original_title !== false) : ?>
                    <p class="link-to-original">
                        <?php printf(esc_html__('Original: %s', 'ailab'), '<a href="' . esc_attr($item->url) . '">' . esc_html($original_title) . '</a>'); ?>
                    </p>
                <?php endif; ?>
                ...
                echo wp_nonce_url(
                    ...
                    'delete-menu_item_' . esc_attr($item_id)
                ); ?>"><?php esc_html_e('Remove', 'ailab'); ?></a> <span class="meta-sep"> | </span> <a
                        ...
                        ?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel', 'ailab'); ?></a>
            </div>
...
class HeroMenuWalker extends Walker_Nav_Menu
{
    ...
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        if (!empty($label)) {
            ...
            switch ($label) {
                case 'hot':
                    $label_text = esc_html__('Hot', 'ailab');
                    break;
                case 'sale':
                    $label_text = esc_html__('Sale', 'ailab');
                    break;
                case 'new':
                    $label_text = esc_html__('New', 'ailab');
                    break;
            }
        ...
        if (!empty($image)) {
            ...
            $label_out = '<img src="' . esc_url($image) . '" class="menu-logo" alt="'.esc_attr__('logo_menu','ailab').'">';
        }
