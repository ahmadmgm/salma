...
class jws_theme_jwsLove {
	...
	function add_love($unit = '',$show_icon = true) {
		...
		return '<a href="#" class="'. esc_attr($class) .'" id="jws-love-'. esc_attr($post->ID) .'" title="'. esc_attr($title) .'"><span class="far fa-heart"></span><span class="count">'.$output .'</span><span class="like_text">'.esc_html($unit).'</span></a>';
	}
