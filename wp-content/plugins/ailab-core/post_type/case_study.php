...
if ( ! defined( 'ABSPATH' ) ) {
    ...
    function jws_register_case_studies() {
		$labels = array(
			'name'                => _x( 'case_study', 'Post Type General Name', 'ailab' ),
			'singular_name'       => _x( 'case_study', 'Post Type Singular Name', 'ailab' ),
			'menu_name'           => esc_html__( 'Case Studies', 'ailab' ),
		...
		$args = array(
			'label'               => esc_html__( 'case_study', 'ailab' ),
		    'labels'              => $labels,
        ...
        if(function_exists('custom_reg_post_type')){
        	custom_reg_post_type( 'case_study', $args );
        }

		/**
		 * Create a taxonomy category for case study
		 *
		...
		$labels = array(
			'name'					=> _x( 'Case Study Categories', 'Taxonomy plural name', 'ailab' ),
			'singular_name'			=> _x( 'Case Study Category', 'Taxonomy singular name', 'ailab' ),
			'search_items'			=> esc_html__( 'Search Categories', 'ailab' ),
			'popular_items'			=> esc_html__( 'Popular Case study Categories', 'ailab' ),
			'all_items'				=> esc_html__( 'All Case study Categories', 'ailab' ),
		...
		$args = array(
            ...
            'query_var'         => true,
            'rewrite'           => array( 'slug' => 'case_study_cat' ),
		);
        

        if(function_exists('custom_reg_taxonomy')){
            custom_reg_taxonomy( 'case_study_cat', array( 'case_study' ), $args  );
        }
        

	};
add_action( 'init', 'jws_register_case_studies', 1 );